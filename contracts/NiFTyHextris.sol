...
contract NiFTyHextris is Context, ERC721 {
    ...
    constructor(
        ...
        randomNumberGeneratorContract = RandomNumberConsumer(
            _randomNumberGeneratorContract
        );
    }

    function startGame() public returns (bytes32 gameID) {
        bytes32 _gameID =
            randomNumberGeneratorContract.getRandomNumber(block.timestamp);
        Game memory newGame =
            Game({
    ...
    function gameOver(
        ...
        Games[msg.sender][_gameID].timeTaken =
            block.timestamp -
            Games[msg.sender][_gameID].startTime;
